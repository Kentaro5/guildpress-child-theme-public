							...
							<?php if( isset($general[$this_year_month_day_stamp]) && count($general[$this_year_month_day_stamp]['register_task']) > 0  ) : ?>
								<?php for ($p=0; $p < count($general[$this_year_month_day_stamp]['register_task']); $p++) : ?>
									...
									<?php if( $this->register_schedule_data !== false || isset( $this->schedule_title ) ) : ?>
										...
										<a class="calendar-title" href="<?php echo $link; ?>" style="color:red;" id="pop_box" onmouseover="calendar_js.show_pop_up('<?php echo $this->schedule_task_id ?>')" onmouseout="calendar_js.hide_pop_up('<?php echo $this->schedule_task_id ?>')" class="p-balloon"><?php echo esc_html($this->schedule_title); ?></a><br/>
